...
import {getLatestVoyagerVersion} from '../services/voyager.service'
...
export const voyenvInit = new Command()
    ...
    .option('-v, --voyager-version <version>', 'voyager version to download', undefined)
...
async function init(folder: string, options: VoyenvInitOptions) {
    ...
    fs.writeFileSync(voyenvFile, template(templateString, {
        ...
        voyagerVersion: options.voyagerVersion ? options.voyagerVersion : await getLatestVoyagerVersion(),
